...
def ssml_to_audio(ssml_text, outfile):
    ...
    with open(outfile, "wb") as out:
        ...
        print(f"{colours.WARNING}Audio content written to file {colours.UNDERLINE}{colours.BOLD}" +
              outfile + f"{colours.REGULAR}")
...
def pickRandName(exclude=None):
    ...
    if exclude:
        ...
        namesListSet = set(names)
        names = namesListSet - excludeSet
...
def createPhrase():
    ...
    phraseRepeated = phrase + \
        "[1] " + repeaters[random.randint(0, len(repeaters)-1)] + " " + phrase
        ...
        if (rndNum == hitNum):
            if lastHit < 2:  # if last hit was recent, dont hit
                ...
                if (random.randint(1, rangeLength) > randomReadHit):
                    addition = randomReads[random.randint(
                        0, len(randomReads)-1)] + "\n"
                else:
...
def callGenerate(path, createAudio, iter, freq, outputnumber=0, outputPath=''):
    ...
    if (not createAudio):
        ...
        ssml_to_audio(ssml, "{0}voices{1}.mp3".format(
            outputPath, outputnumber))
...
if __name__ == '__main__':
    ...
    print("~~~~~~~~~~~~~~~~~~~")
    print(f"create audio? {colours.GREEN}" +
          str(createAudio)+f"{colours.REGULAR}")
    print(f"# iterations? {colours.GREEN}" +
          str(iterations)+f"{colours.REGULAR}")
    print(f"freq of phrases? {colours.GREEN}" +
          str(frequency)+f"{colours.REGULAR}")
    print("~~~~~~~~~~~~~~~~~~~")
    ...
    # # # # # # # # #
    numPerOutput = 50
    # only used when iterations exceeds 'numPerOutput'
    loopAmount = (iterations//numPerOutput)
    ...
    if (iterations > numPerOutput):
        ...
        for i in range(0, loopAmount):
            print(str(i+1)+" loop")
            callGenerate(guest_list_path, createAudio, numPerOutput,
                         frequency, i, outputPath=voicesDir)
        if (remaining > 0):
            print("")
            print("~~~~~~~~{} calls remaining:".format(remaining))
            callGenerate(guest_list_path, createAudio, remaining,
                         frequency, '-LAST', outputPath=voicesDir)

    else:  # just call as normal, no output number required.
        callGenerate(guest_list_path, createAudio, iterations,
                     frequency, outputPath=voicesDir)
